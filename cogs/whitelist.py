...
class WhitelistCog(commands.Cog, name='Whitelist'):
    ...
    async def set_whitelist_channel(
        ...
        self.bot.logger.info(
            f'User: {ctx.author.mention} (`@{ctx.author}`) has set the whitelist channel to channel: `#{channel.name}` in guild: `{ctx.guild.name}`.'
        )
    ...
    async def toggle_whitelist(self, ctx: discord.ApplicationContext) -> None:
        ...
        self.bot.logger.info(
            f"User: {ctx.author.mention} (`@{ctx.author}`) has {'enabled' if whitelist.enabled == False else 'disabled'} the whitelist for channel: `#{channel.name}` in guild: `{ctx.guild.name}`."
        )
